...
namespace SyncBlink
{
    ...
    private:
        ...
        std::unique_ptr<const AstNode> parseExpression(size_t precedence);
        std::unique_ptr<const AstNode> parseBlockExpression();
        std::unique_ptr<const AstNode> parseArrayAssignExpression();
        ...
        std::unique_ptr<const AstNode> parseAssignExpression(std::unique_ptr<const AstNode> left, Token lastToken);
        ...
        Scanner _scanner;
        std::tuple<size_t, std::string> _parserError = std::make_tuple(0, "");
