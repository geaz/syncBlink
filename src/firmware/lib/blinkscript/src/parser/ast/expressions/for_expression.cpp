...
namespace SyncBlink
{
    ForExpression::ForExpression(std::unique_ptr<const AstNode> assignStatement, std::unique_ptr<const AstNode> conditionExpression,
                                 std::unique_ptr<const AstNode> incrementorStatement, std::unique_ptr<const AstNode> loopBody,
                                 size_t line)
    ...
    size_t ForExpression::getLine() const
    {
        return _line;
    }
}