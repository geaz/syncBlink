...
namespace SyncBlink
{
    namespace Api
    {
        FrequencyAnalyzer::FrequencyAnalyzer(uint64_t analyzerId, MessageBus& messageBus) : _analyzerId(analyzerId), _messageBus(messageBus)
        {
        }
        ...
        std::array<float, MaxFreqBinIndex> FrequencyAnalyzer::fftToSmoothedNormalizedFftDb(const kiss_fft_cpx* cx,
                                                                                           uint16_t& dominantFrequency)
        {
            ...
            for (uint16_t i = 0; i < MaxFreqBinIndex; i++)
            {
                ...
                if (fftCurr != 0) smoothedFft[i] = EfAlpha * _prevSmoothedFft[i] + ((1.0f - EfAlpha) + fftCurr);
                else
                    smoothedFft[i] = 0;
        ...
        /* static */ int FrequencyAnalyzer::getAnalyzerResult(void* outputBuffer, void* inputBuffer, unsigned int nBufferFrames,
